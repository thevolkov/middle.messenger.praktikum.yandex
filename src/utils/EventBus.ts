export default class EventBus {
  ...
  public off (event: string, callback: (...args: unknown[]) => void): void {
    ...
    this.listeners[event] = this.listeners[event].filter((listener: (...args: unknown[]) => void) => listener !== callback)
  }

  public emit (event: string, ...args: unknown[]): void {
    if (this.listeners[event] == null) throw new Error(`Нет события: ${event}`)

    this.listeners[event].forEach((listener: (...args: unknown[]) => void) => {listener(...args)})
  }
}
